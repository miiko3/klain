...
**klain (клайн)** — это песочница для запросов к разным ИИ, к которым у пользователя уже есть доступ по API. Для iPhone и iPad с **iOS 17+**.

**Генерация фото, видео, аудио и прочего недоступна.** klain — текстовый чат-клиент: он отправляет запросы к выбранным текстовым моделям и показывает ответы.

**Где взять API:** ключ можно приобрести через рекламный блок в приложении либо у сторонних продавцов API-доступа. klain не продаёт токены и не выдаёт ключи.
...
Генерация изображений, видео, аудио и другого медиа в klain недоступна. Приложение может отображать структурированные изображения и ссылки на видео, если они уже возвращаются поддерживаемыми чат-API, но это не отдельная генерация медиа. Отдельные API генерации видео, аудио и нативные генераторы Google/xAI не реализованы. Для CheapVibeCode генерация медиа не подтверждена.
...
Автор: **[miiko3](https://github.com/miiko3)** · [Threads @klain_app](https://www.threads.com/@klain_app) · [Telegram @yetilov](https://t.me/yetilov) · [Boosty](https://boosty.to/miilo3)
